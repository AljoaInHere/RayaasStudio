...
<head>
    ...
    <title>Payment Setup Package - Raya Studio</title>
...
<div class="flex flex-col md:flex-row h-screen w-full overflow-hidden payment-container">
    ...
    <div class="w-full md:w-1/2 bg-gradient-to-br from-[#090916] via-[#150926] to-[#22093a] border-b md:border-b-0 md:border-r border-white/8 text-white flex flex-col justify-center items-center text-center p-10 md:p-[60px] relative overflow-hidden payment-left">
        <div class="product-image-checkout">
            @if($package->image)
                <img src="/storage/{{ $package->image }}" alt="{{ $package->name }}">
            @else
                <div class="no-image">⚙️</div>
            @endif
        </div>
        <h2>Checkout Setup Package</h2>
        <p class="product-name">{{ $package->name }} Package</p>
    ...
    <div class="w-full md:w-1/2 bg-bg-base flex flex-col justify-center p-10 md:p-[60px_80px] payment-right animate-fade-up">
        ...
        <form id="paymentForm" action="{{ route('payment.setup.process', $package->id) }}" method="POST">
            @csrf
            ...
            <input type="text" id="paket" value="{{ $package->name }} Package" readonly>
        ...
        <div class="back-link">
            <a href="{{ route('setup.index') }}">← Kembali</a>
        </div>
    </div>
</div>
...
<div id="confirmOverlay" class="confirm-overlay">
    <div class="confirm-card">
        ...
        <div class="confirm-detail">
            ...
            <span class="value" style="color: var(--primary);">Rp {{ number_format($package->price, 0, ',', '.') }}</span>
        </div>
        <div class="confirm-detail">
        ...
        <div class="confirm-actions">
            ...
            <button class="btn btn-primary" onclick="processPayment()">Konfirmasi</button>
        </div>
    </div>
</div>
...
function showConfirmation() {
    ...
    if (!metodeValue) { metode.focus(); return; }
...
function processPayment() {
    ...
    if (metode === 'qris') {
        ...
        window.location.href = '/payment/qris/setup/' + packageId + '?nama=' + nama;
    } else {
